...
export default function MainMenu() {
  ...
  const MenuButton = ({ to, text }: { to: string; text: string }) => {
    return (
      <Button variant='contained' color='success' size='large' sx={{ width: '98%', fontSize: '1.2rem' }} component={Link} to={to}>
        {text}
      </Button>
    )
  }
    ...
    >
      <Grid container sx={{ marginBottom: 1 }} alignItems='flex-start' justifyContent='flex-start'>
        <Grid container item xs={4} spacing={1}>
          <Grid item xs={12}>
            <MenuButton text='Create new game' to='/new-game' />
          </Grid>
          <Grid item xs={12}>
