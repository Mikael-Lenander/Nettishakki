...
class Pos {
    ...
    squaresBetween(other) {
        ...
        if (!isFinite(slope)) {
            return (0, lodash_1.range)(min.y + 1, max.y).map((y) => new Pos(this.x, y));
        }
        else if (slope === Math.sign(slope)) {
            const yStart = this.x < other.x ? this.y : other.y;
            return (0, lodash_1.range)(min.x + 1, max.x).map((x, index) => new Pos(x, yStart + slope * (index + 1)));
        }
        return [];
    }
    toString() {
    ...
    static new(posType) {
        if (typeof posType === 'string')
            return new Pos('abcdefgh'.indexOf(posType[0]), parseInt(posType[1]) - 1);
